...
class HomeController extends Controller
{
    ...
    public function index(){
        // hiện thị sách với đánh giá, khuyến mãi
        $list=  DB::table('dausach')->get();
        $array_sach=[];
        ...
        //sách bán chạy
        $list_sach=  DB::table('donhang')
        ->selectRaw('round(avg(danhgia.diem_dg)) as diemtb,sum(ctgiohang.so_luong) as soluong, MONTH(ngay_dat) as thang, YEAR(ngay_dat) as nam, ctgiohang.id_sach, dausach.ten_sach, dausach.hinh_anh, dausach.gia_sach')
    ...
    public function chitiet($sach_id){
        ...
        return view('home.page.chitiet')->with('danh_gia',$array)->with('tong_dg',$tong_dg)->with('kh_dg',$kh_dg)->with('danhgia',$rating)->with('sach',$tl_sach)->with('sach_tuongtu',$sach_tt);
    }
    ...
    public function TL1_sach(Request $request){
        ...
        if($request->get('tacgia')){
            $tg_sach= DB::table('dausach')->join('tacgia','dausach.id_tg','=','tacgia.id_tg')->where('dausach.id_tg',$request->get('tacgia'))->paginate(12);
            return view('home.page.loc_sach')
            ->with('list_tacgia',$list_tacgia)
            ->with('list_theloai',$list_theloai)
            ->with('dausach',$tg_sach);
        }elseif($request->get('theloai')){
            $tl_sach= DB::table('dausach')->join('theloai','dausach.id_tl','=','theloai.id_tl')->where('dausach.id_tl',$request->get('theloai'))->paginate(12);
            return view('home.page.loc_sach')
            ->with('list_tacgia',$list_tacgia)
            ->with('list_theloai',$list_theloai)
            ->with('dausach',$tl_sach);
        }elseif($request->get('danhgia')){
            $danh_gia= DB::table('dausach')
            ->selectRaw('round(avg(danhgia.diem_dg)) as diemtb, dausach.id_sach, dausach.hinh_anh, dausach.ten_sach, dausach.gia_sach')
            ->join('theloai','dausach.id_tl','=','theloai.id_tl')
            ->join('danhgia','dausach.id_sach','=','danhgia.id_sach')
            ->groupBy('dausach.id_sach') 
            ...
            $gia_sach= DB::table('dausach')->whereBetween('gia_sach', [$tu,$den])->paginate(12);
            return view('home.page.loc_sach')
            ->with('list_tacgia',$list_tacgia)
            ->with('list_theloai',$list_theloai)
            ->with('dausach',$gia_sach);
        }elseif($request->get('search')){
            $tk_sach= DB::table('dausach') 
            ...
            ->with('list_tacgia',$list_tacgia)
            ->with('list_theloai',$list_theloai)
            ->with('dausach',$tk_sach);
        }else{
            $tk_sach= DB::table('dausach')->paginate(12);
            return view('home.page.loc_sach')
            ->with('list_tacgia',$list_tacgia)
            ->with('list_theloai',$list_theloai)
            ->with('dausach',$tk_sach);
