...
<body class="js">
	...
	<script>
		jQuery(document).ready(function() {
			...
			$('#danhgia').on('click', function() {
				...
				for (var i = 0; i < checkbox.length; i++) {
					if (checkbox[i].checked === true) {
						var diem_dg = checkbox[i].value;
					}
				}
				var id_sach = $('.sach-id').val();
				var noi_dung = $('textarea#noi-dung').val();
				if (diem_dg)
					$.ajax({
						url: "{{ url('danh-gia') }}",
						type: "POST",
			...
			$('.resetmk').on('click', function() {
				var token = $('.tokenmk').val();
				var email = $('.emailmk').val();
				var password = $('.passmk').val();
				if (token)
					$.ajax({
						url: "{{ url('/datlai-mk') }}",
						type: "POST",
			...
			$('.js-danh-gia').on('click', function() {
				if ($("#rating-action").hasClass('hide')) {
					...
					$("#rating-action").addClass('hide').removeClass('active');
				}
		...
		function remove_background(sach_id) {
			for (var n = 1; n <= 5; n++) {
				$('#' + sach_id + '-' + n).css('color', '#ccc');
			}
		};

		// jQuery(document).on('mouseleave','.rating',function(){
		// 	var index = $(this).data("index");
		// 	var sach_id = $(this).data("sach-id");
		// 	var rating = $(this).data("rating");
		// 	remove_background(sach_id)

		// });
		function save_up(rowId) {
			const x = parseInt($("#qty-" + rowId).val());
			var a = x + 1;

			$.ajax({
				url: "{{ url('save-list-cart/') }}/" + rowId + '/' + a,
				type: "GET",
			}).done(function(response) {
				$("#list-cart").empty();
				$("#list-cart").html(response);
